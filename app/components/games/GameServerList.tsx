        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {currentGame?.plans[selectedPlanType].map((plan: GamePlan, index: number) => (
            <motion.div
              ...
              style={{ "--game-color": plan.tierColor || currentGame?.primaryColor } as React.CSSProperties}
            ...
            >
              <div className="absolute top-3 right-3 flex flex-col items-end gap-1">
                ...
                {typeof plan.stock === 'number' && (
                  <div
                    className="px-2 py-1 text-xs font-semibold rounded-full border"
                    style={{
                      color: 'var(--game-color)',
                      borderColor: 'color-mix(in srgb, var(--game-color) 40%, transparent)',
                      backgroundColor: 'color-mix(in srgb, var(--game-color) 15%, transparent)',
                    }}
                  >
                    {plan.stock <= 3 ? `Only ${plan.stock} left` : `Stock: ${plan.stock}`}
                  </div>
                )}
              </div>
              <div className="p-6">
